...
- **`sec-edgar`** Atom feeds use per-form watermarks (`sec-edgar:8-K`, `sec-edgar:4`, …) and
  `start=` pagination (up to 10×100) when a page is full of unknown accessions — so Form 4/8-K
  bursts cannot silently roll off `getcurrent`. If a tick still hits max pages without
  catching up, the form watermark is **held** (not advanced). EOD **daily-index**
  (`master.YYYYMMDD.idx`) reconciles via `sec-edgar:daily-index` for multi-hour/day gaps.
