...
**One separator only.** Use a single colon between company and event. Put any tagline in parentheses — never a second dash / em dash:
...
| Wire / news                                          | Story headline as-is (no `{Company}: {Event}` rewrite)                                                  |
