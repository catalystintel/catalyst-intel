...
import { extractFiledDate, parseFilingTitle, stripHtml } from "./fetch-sec-edgar";
...
describe("parseFilingTitle", () => {
  ...
  it("parses a title with a dash in the company name", () => {
    expect(parseFilingTitle("10-Q - SOBR Safe, Inc. (0001850079) (Filed by)")).toEqual({
      formType: "10-Q",
...
describe("extractFiledDate", () => {
  it("extracts a filed date from summary text", () => {
    expect(extractFiledDate("Acc-no: 0001234567-26-000123 (Filed: 2026-07-17)")).toBe(
      "2026-07-17",
    );
  });
