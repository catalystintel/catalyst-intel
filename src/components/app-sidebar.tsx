 ...
 * Collapsed mode is an icon rail with instant DeskTip labels (no native
 * `title=` delay).
 */
export function AppSidebar({
  ...
  return (
    ...
    >
      <SidebarTip content="Catalyst Feed" enabled={collapsed} className="mb-5">
        <Link
          href="/catalyst-feed"
          onClick={onNavigate}
          ...
          className={cn(
            ...
            collapsed ? "size-full justify-center" : "mb-5 px-2 py-1",
          )}
        >
          <span
            aria-hidden
          ...
          >
            <button
              ...
              className={cn(
                ...
                collapsed ? "size-full justify-center" : "w-full px-2.5 py-2",
              )}
            >
              {collapsed ? (
...
function SidebarEntry({
  ...
  const base = cn(
    ...
    collapsed ? "size-full justify-center" : "w-full gap-2.5 px-2.5 py-2",
  );
  ...
  if (item.comingSoon || !item.href) {
    ...
    return (
      <SidebarTip content={hint} enabled={collapsed}>
        {body}
      </SidebarTip>
    );
  }
  ...
  return (
    <SidebarTip content={item.label} enabled={collapsed}>
      {link}
    </SidebarTip>
  );
}
