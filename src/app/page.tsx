...
const DEMO_ROWS = [
  { ticker: "NVDA", type: "8-K", age: "12s", why: "Item 2.02 — preliminary quarterly results" },
  { ticker: "TSLA", type: "8-K", age: "48s", why: "Item 8.01 — other events, guidance update" },
  { ticker: "AMD", type: "8-K", age: "2m", why: "Item 1.01 — material definitive agreement" },
  { ticker: "PLTR", type: "8-K", age: "5m", why: "Item 5.02 — officer / director change" },
  { ticker: "SMCI", type: "8-K", age: "9m", why: "Item 2.05 — costs associated with exit" },
] as const;
...
export default async function Home() {
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden">
      ...
      <main className="page-enter relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center gap-10 px-5 pb-16 pt-4 sm:px-8">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 max-w-lg text-pretty text-base text-muted-foreground sm:text-lg">
            Live SEC catalysts as they hit the wire — built for multi-monitor scanning,
            not another SaaS dashboard. Sign in and land on the feed.
          </p>
        ...
        >
          <div className="flex items-center justify-between border-b border-border/60 px-3 py-2.5 sm:px-4">
            ...
            <span className="font-mono text-[0.65rem] text-muted-foreground">Preview</span>
          </div>
          <div className="grid grid-cols-[4.5rem_3.5rem_2.75rem_minmax(0,1fr)] gap-2 border-b border-border/50 px-3 py-2 font-mono text-[0.6rem] uppercase tracking-[0.12em] text-muted-foreground sm:grid-cols-[5.5rem_4rem_3rem_minmax(0,1fr)] sm:px-4">
            <span>Ticker</span>
            <span>Type</span>
          ...
          <ul className="divide-y divide-border/40">
            {DEMO_ROWS.map((row, index) => (
              ...
              >
                ...
                <span className="font-mono text-[0.7rem] text-muted-foreground">{row.type}</span>
                <span className="text-right font-mono text-[0.7rem] tabular-nums text-amber-200/85">
                  {row.age}
                </span>
                <span className="truncate text-[0.8rem] text-foreground/85">{row.why}</span>
              </li>
            ))}
          </ul>
        </section>
